...
new %>% group_by(lap) %>% summarise(patients=n_distinct(id_patients), procedures=n()) %>% kable(col.names=c("Laparoscopic", "Patients", "Procedures"))

#Single ops
new %>% group_by(id_patients) %>% filter(n()==1) %>% nrow() -> singlepts

#multiples
new %>% group_by(id_patients) %>% filter(n()>1) %>% tally() %>% nrow() -> multiplepts
