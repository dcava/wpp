...
library(lubridate)
library(mice)
source("../../../Research/colorectal mets/scripts/stable.weights.R")
library(twang)
library(dplyr)
library(tidyr)
library(magrittr)
...
#Print demographics
kable(ptnumbers, col.names=c("Laparoscopic", "Patients", "Procedures"))
...
kable(majorhep, col.names = c("Laparoscopic", "Hepatectomy", "n"), caption = "Major hepatectomy")
kable(lls, col.names = c("Laparoscopic", "n"), caption = "Left lateral sectionectomy")
kable(segment, col.names = c("Laparoscopic", "n"), caption = "Segmentectomy/Bisegmentectomy")
kable(tumorectomy, col.names = c("Laparoscopic", "n"), caption = "Tumorectomy")
```
...
for(i in 1:5) {
      tmp <- ps(lap~age+sex+CEA+ismajor+difloc+isbilateral+lesionmaxdiameter+lesioncount+primarytumourgrade+primaryT+primaryN+year_primary+primarytreatment, estimand = "ATE", data=complete(impmids,i), interaction.depth=2, n.trees = 1000, shrinkage=0.05, verbose=FALSE, bag.fraction = 0.3, stop.method = "ks.max")
...
impdata[["psate0"]] <- as.data.frame(subnew)
  for (i in 1:5) {
      ...
      impweights[[name]] <- data.frame(weights=stable.weights(propscores[[i]], "ks.max"), treat=propscores[[i]]$treat)
  }
...
plotweights <- data.frame(weights=c(unlist(list.map(impweights, weights))), treat=c(unlist(list.map(impweights, treat))), imp=rep(1:5, each=284))
...
postopwtd <- MIcombine(with(design.ate, svyby(~bloodloss+hlos++margin, ~lap,svyquantile, quantiles=0.5, ci=TRUE, vartype = "var")))
