...
#Consolidate and calculate stabilised weights. Add them to the imputed datasets.
```{r, results='hide', warning=FALSE, error=FALSE, message=FALSE, echo=FALSE}
...
#Convert the list of datasets to one large dataframe (for convenience and plotting) and re-create a "mids" object for later use. Generate the conditional/calculated variables. 
```{r, results='hide', warning=FALSE, error=FALSE, message=FALSE, echo=FALSE}
...
#Final imputed mids
midata <- as.mids(as.data.frame(finaldata), .imp = match(".imp", names(finaldata)), .id=match(".id", names(finaldata)))
rm(impmids, impweights, imptmp)
```

#MItools version - this version is necessary for some of the weighted statistics. Same data, slightly different structure to work with a different package.
```{r, results='hide', warning=FALSE, error=FALSE, message=FALSE, echo=FALSE}
#mitools version
library(mitools)
mit <- impdata[2:6]
...
#Survey design - these design objects describe the data to the survey package so it can make appropriate adjustments to errors based on the clustering.
```{r, results='hide', warning=FALSE, error=FALSE, message=FALSE, echo=TRUE}
...
#5yr OS by group
fiveyros <- survfit(Surv(cens_time, cens)~lap+cluster(id_patients), weights = stable.w, midata$imputations$psate1, subset=index==1)
...
summary(MIcombine(doublerobust.full))

doublerobust.final <- with(midata, coxph(Surv(time,ctime,cens)~lap+cluster(id_patients), weights = stable.w))

summary(MIcombine(doublerobust.final))

#Compare HR for lap

#PDFS - PWP-CT model
rec_double.full <- with(midata, coxph(Surv(time,rtime,(rec==1|cens==1))~lap+ismajor+isbilateral+primaryN+lesioncount + strata(index), weights = stable.w))

summary(MIcombine(rec_double.full))

rec_double.final <- with(midata, coxph(Surv(gaptime,(rec==1|cens==1))~lap + strata(index), weights = stable.w))

summary(MIcombine(rec_double.final))
...
summary(MIcombine(doublerobust.mvr))

########Final multivariate models

os_final <- with(midata, coxph(Surv(time,ctime,cens)~cluster(id_patients)+mhlos+primaryT+lap, weights = stable.w))

summary(MIcombine(os_final))

rec_doublerobust.mvr <- with(midata, coxph(Surv(gaptime,(rec==1|cens==1))~lap+cluster(id_patients)+ismajor+isbilateral+primaryN+lesioncount+primaryT+bloodloss+mhlos+optime+strata(index), weights = stable.w))

#summary(pool(rec_doublerobust.mvr))

rfs_final <- with(midata, coxph(Surv(gaptime, (rec==1|cens==1))~cluster(id_patients)+mhlos+primaryT+isbilateral+lap, weights = stable.w))

#summary(pool(rfs_final))

os.final.data <- as.data.frame(summary(pool(as.mira(os_final))))
...
rfs.final.data <- as.data.frame(summary(pool(as.mira(rfs_final))))
