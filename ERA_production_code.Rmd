...
#KM survival by operative approach (in days)
...
#Peto-Peto modification of Gehan-Wilcoxon test (log rank), lichtrub p-val test (not perfect)
lros <- with(design.ate, svylogrank(Surv(cens_time, cens)~lap))

qbarX2 <- sum(unlist(list.select(list.map(lros, list.extract(.,2)),chisq)))/5
pbar <- unlist(list.select(list.map(lros, list.extract(.,2)),p))
...
qbarX2 <- str_c("Log-rank test (X2=", round(qbarX2,2), " p = ", round(pbar,2), ")")
...
#logrank RFS---------
lrrfs <- with(design.ate, svylogrank(Surv(gaptime, (rec==1|cens==1))~lap+strata(index)))

qbarX2rfs <- sum(unlist(list.select(list.map(lrrfs, list.extract(.,2)),chisq)))/5
pbarrfs <- unlist(list.select(list.map(lrrfs, list.extract(.,2)),p))

#lichtrubin
z <- qnorm(pbarrfs)
num <- mean(z)
den <- sqrt(1 + var(z))
pbarrfs <- pnorm( num / den)

qbarX2rfs <- str_c("Log-rank test (X2=", round(qbarX2rfs,2), " p = ", round(pbarrfs,2), ")")
#---------------
...
pander(qbarX2)
pander(cohortmedos)
pander(str_c("Open ", fiveyropen))
pander(str_c("Laparoscopic ", fiveryrlap))

#Median RFS 
pander(pool.km(rfskm), caption = "Kaplan-Meier RFS")
pander(qbarX2rfs)
#5yr RFS
...
pander(spm(mvr.rfs)) #full rfs multivar
pander(summary(mvr.os.raw))
pander(summary(mvr.rfs.raw))

pander(spm(os_final)) #final os multivar
pander(spm(rfs_final)) #final rfs multivar
pander(raw.os.final)
pander(raw.rfs.final)
```
